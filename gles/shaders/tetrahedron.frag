...
void main() {
    float iGlobalTime = 4.0;
    vec2 iResolution = vec2(300., 300.);
    vec4 iMouse = vec4(300., 300., 0., 0.);
    vec2 _fragCoord = vec2(300., 300.);
    _fragCoord = v_texCoord;
    vec2 originalPos = (600.0 * _fragCoord - iResolution.xy)/iResolution.yy;
    ...
    // Background
	tempColor = vec4(_BackgroundColor.rgb * (1.0-0.2*length(originalPos)), 1.);
    for (int i = 5; i >= 0; i--) {
        tempColor = mix(tempColor, layers[i], layers[i].a);
    }
    
    for (int i = 0; i < 4; i++) {
        if (circleColors[i] > 0.) {
            vec4 c = circle_with_color(pos, p[i].xy, _CircleRadius, _FrontColor);
            tempColor = mix(tempColor, c, c.a);
        }
    }
