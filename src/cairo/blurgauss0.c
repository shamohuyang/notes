/* https://people.freedesktop.org/~ranma42/blurgauss0.c */
...
static void
patch_out_to_gauss (cairo_pattern_t *pattern,
		    double x0, double y0,
		    double x1, double y1)
{
    cairo_mesh_pattern_curve_to (pattern,
				 LERP (x0, x1, c0), LERP (y0, y1, c0),
				 LERP (x0, x1, c1), LERP (y0, y1, c1),
				 x1, y1);
}

static void
patch_in_to_gauss (cairo_pattern_t *pattern,
		    double x0, double y0,
		    double x1, double y1)
{
    cairo_mesh_pattern_curve_to (pattern,
	    ...
	    double r, double g, double b, double a)
{
    ...
    patch_out_to_gauss (pattern, x0, y0, x0 + dx, y0 + dy);
    cairo_mesh_pattern_line_to (pattern, x1 + dx, y1 + dy);
    patch_in_to_gauss (pattern, x1 + dx, y1 + dy, x1, y1);
...
int main(int argc, char **argv)
{
    ...
    cairo_surface_write_to_png (surface, "blurgauss.png");
